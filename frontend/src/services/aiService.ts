...
export async function generateMeetingSummary(
  ...
  try {
    ...
    // generatePrompt is now async, so we need to await it
    const prompt = await generatePrompt(transcript);
...
/**
 * Default prompt for summarizing a meeting transcript
 */
const DEFAULT_ZOOMARY_PROMPT = `
  You are an expert meeting summarizer. Please create a comprehensive, detailed, and professional summary 
  of the following meeting transcript. Your summary should be:
...
/**
 * Fetch the custom prompt from AI agent settings or use the default
 */
async function fetchCustomPrompt(agentName: string): Promise<string | null> {
  try {
    const response = await fetch("/aiagentsettings");
    if (!response.ok) {
      console.warn(`Failed to fetch AI agent settings: ${response.status}`);
      return null;
    }

    const data = await response.json();
    if (!data?.data || !Array.isArray(data.data)) {
      console.warn("Invalid AI agent settings data format");
      return null;
    }

    const agentSetting = data?.data?.find(
      (setting) => setting.name === agentName
    );
    if (!agentSetting || !agentSetting.promptContent) {
      console.warn(`No custom prompt found for ${agentName}`);
      return null;
    }

    return agentSetting.promptContent;
  } catch (error) {
...
/**
 * Generate a prompt for summarizing a meeting transcript
 * Uses custom prompt from settings if available, otherwise falls back to default
 */
async function generatePrompt(transcript: string): Promise<string> {
  // Try to get custom prompt from settings
  const customPrompt = await fetchCustomPrompt("Zoomary");

  // Use custom prompt if available, otherwise use default
  const promptTemplate = customPrompt || DEFAULT_ZOOMARY_PROMPT;
