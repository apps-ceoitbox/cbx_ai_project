...
export function constructAlternativeVttUrl(zoomUrl: string): string | null {
  try {
    ...
    const vttUrl = `${baseVttUrl}?type=transcript&fid=${meetingId}${
 ...
 */
export async function extractVttUrlFromZoomRecording(
  zoomUrl: string
): Promise<string | null> {
  try {
  ...
  try {
    ...
    try {
      ...
      if (directResponse.ok) {
        ...
        if (
          ...
          console.warn(
            "Direct response doesn't look like VTT, will try proxies"
          );
        }
      }
    } catch (err) {
    ...
    for (const proxyUrl of corsProxies) {
      try {
        ...
        if (!response.ok) {
          ...
          proxyDetailsForDebug.push(
            `${proxyUrl.substring(0, 30)}... - ${statusInfo}`
          );
          continue;
        }
        ...
        if (
          ...
          proxyDetailsForDebug.push(
            `${proxyUrl.substring(0, 30)}... - Invalid content type`
          );
        }
      } catch (err) {
        console.warn(`Proxy ${proxyUrl} failed with error:`, err);
        proxyDetailsForDebug.push(
          `${proxyUrl.substring(0, 30)}... - ${
    ...
    if (!vttContent) {
      const detailedError = new Error(
        `All fetch methods failed. Details: ${proxyDetailsForDebug.join(" | ")}`
      );
...
export async function directZoomVttFetch(zoomUrl: string): Promise<string> {
  try {
    ...
    const ccVttUrl = `https://zoom.us/rec/play/vtt?type=cc&fid=${meetingId}${
      accessToken ? "." + accessToken : ""
    }&action=play`;
    const transcriptVttUrl = `https://zoom.us/rec/play/vtt?type=transcript&fid=${meetingId}${
      accessToken ? "." + accessToken : ""
    }&action=play`;
